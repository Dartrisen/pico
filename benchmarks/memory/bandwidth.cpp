...
void bench_random_bandwidth()
{
    ...
    for (size_t kb : sizes_kb)
    {
        ...
        std::cout << "Size: " << std::setw(5) << kb << " KB | Accesses/sec: " << std::scientific << std::setprecision(3)
                  << std::setw(10) << ops_per_sec << " | "
                  << "Latency: " << std::fixed << std::setprecision(3) << std::setw(6) << (1.0 / (ops_per_sec / 1e9))
                  << " ns\n";
