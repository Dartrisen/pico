...
int main()
{
    ...
    auto linear_density = [=](double x) -> float
    {
        ...
        if (x < x_ramp_in)
            return static_cast<float>((x - x_start) / ramp_buffer) * n_start;
        if (x > x_ramp_out)
            return static_cast<float>((x_end - x) / ramp_buffer) * n_end;

        const float frac = static_cast<float>((x - x_ramp_in) / (x_ramp_out - x_ramp_in));
        return n_start + (n_end - n_start) * frac;
    };

    // Add Species 0: Thermal Electrons
    engine_instance.add_species_thermal(ppc, linear_density, v_thermal_e,
                                        /*base_charge=*/-1.0f,
                                        /*base_mass=*/1.0f,
    ...
    engine_instance.add_species_thermal(ppc, linear_density, v_thermal_p,
                                        /*base_charge=*/1.0f,
    ...
    pico::ui::VerificationReport report("Linear Density Proton-Electron Energy Conservation Verification", res.passed);
